...
export const PricingWrapper: React.FC<{
  ...
  <article
    className={cn(
      "min-h-[420px] w-full relative overflow-hidden rounded-2xl text-white",
      colorClass,
      className
    )}
  >
    <span className="w-full h-full absolute top-0 left-0 z-[2] p-6 flex flex-col items-start justify-start gap-4">
      {children}
      <div className="w-full mt-auto flex items-end justify-end text-base">
        {href ? (
          <Link href={href} className="w-full h-fit">
            <button className="h-12 w-full bg-white rounded-lg text-neutral-900 font-bold hover:bg-neutral-100 transition-colors">
              {buttonText}
            </button>
          </Link>
        ) : onClick ? (
          <button
            onClick={onClick}
            className="h-12 w-full bg-white rounded-lg text-neutral-900 font-bold hover:bg-neutral-100 transition-colors"
          >
            {buttonText}
          </button>
        ) : null}
...
export const Heading: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <h1
    className={cn("sm:text-5xl leading-[1] text-[clamp(1.7rem,10vw,3rem)] font-bold", className)}
  >
    {children}
  </h1>
);
...
export const Paragraph: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <p className={cn("sm:text-xl text-[clamp(0.1rem,20vw,1.1rem)] font-medium leading-relaxed", className)}>
    {children}
  </p>
);
